...
- (void)showLineWithButtonWidth:(CGFloat)width x:(CGFloat)x
{
    _line = [[UIView alloc] initWithFrame:CGRectMake(x + 2.0f, NAV_TAB_BAR_HEIGHT - 3.0f, width - 4.0f, 3.0f)];
...
- (CGFloat)contentWidthAndAddNavTabBarItemsWithButtonsWidth:(NSArray *)widths
{
    ...
    CGFloat kuan = 0.0f;
    for (NSNumber *kuans in widths) {
        kuan += [kuans floatValue];
    }
    if (kuan <= SCREEN_WIDTH) {
        buttonX = SCREEN_WIDTH/2.0 - kuan/2.0;
    } else {
        buttonX = DOT_COORDINATE;
    }
    
    [self showLineWithButtonWidth:[widths[0] floatValue] x:buttonX];
    ...
    for (NSInteger index = 0; index < [_itemTitles count]; index++)
    {
        ...
        [button setTitleColor:[UIColor blackColor] forState:UIControlStateNormal]; // 标题字体颜色
